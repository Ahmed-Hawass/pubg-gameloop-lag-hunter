...
| --- | --- |
| `npm test` | Vitest unit tests (22 across 5 files), version comparison (the update check), error-to-dialog mapping, locale key parity, theme resolution |
| `npm run build` | TypeScript compiles (incl. the locale type bond: `ar.ts` must match `en.ts` key-for-key) and the frontend bundles |
| `cargo test` | All 76 tests (74 unit + 2 live integration), detector fingerprints, settings migration, path traversal, honest reporting, plus the **live** typeperf/PowerShell pipeline tests |
